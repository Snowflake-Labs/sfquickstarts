...
def build_content_fragment_payload(
    ...
    if open_in_snowflake_link:
        form_data.append(
            ("./data/master/quickstartArticleOpenInSnowflakeLink", open_in_snowflake_link)
        )
    ...
    custom_last_modified = f"{datetime.now(UTC).strftime('%Y-%m-%d')}T00:00:00.000Z"
...
def build_page_payload(jcr_title: str, content_fragment_path: str, tags: list[str]) -> str:
    """
    ...
    custom_pub_date = f"{datetime.now(UTC).strftime('%Y-%m-%d')}T00:00:00.000Z"
...
def prepare_aem_payload(
    parsed_markdown: dict[str, Any], content_fragment_path: str, language: str = "en"
) -> dict[str, Any]:
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare AEM payload from parsed markdown JSON")
    parser.add_argument(
        "input_json", help='Path to JSON file from parse_markdown.py or "-" for stdin'
    )
    parser.add_argument(
        "--content-fragment-path", required=True, help="Full AEM path for the content fragment"
    )
    ...
    try:
        ...
        result = prepare_aem_payload(parsed_markdown, args.content_fragment_path, args.language)
