...
summary: demonstrates Horizon features for intra-company sharing of data and applications 
...
- Switch to the provider account where you published the listing
...
- Switch back to your consumer account Horizon_Lab_AWS_Consumer to see that the data changes are instantly visible. For example:
...
SELECT * EXCLUDE city 
FROM analytics.CUSTOMER_LOYALTY_METRICS_V
WHERE city IN ('Dehli','Tokyo','Seoul','Melbourne','Sydney','Mumbai');
...
Note that a projection policy by itself does not prevent users from detecting information about individuals. For example, the following queries are permitted:
...
But, let's setup a task that moves the latest quality metrics into a regular table for sharing, e.g. the last 7 days worth of quality metrics. 
...
CREATE OR REPLACE TASK FROSTBYTE_TASTY_BYTES.dq.subset_quality_events
   SCHEDULE = '3 MINUTE'
   AS BEGIN
        DELETE FROM FROSTBYTE_TASTY_BYTES.dq.shared_quality_events
               WHERE measurement_time < current_date - 7;

        MERGE INTO  FROSTBYTE_TASTY_BYTES.dq.shared_quality_events t1 
              USING snowflake.local.data_quality_monitoring_results t2
               ON t1.measurement_time = t2.measurement_time 
              AND t1.metric_name = t2.metric_name 
        WHEN NOT MATCHED 
        THEN INSERT (scheduled_time, measurement_time, table_name, table_schema, 
                     table_database, metric_name, metric_schema, argument_names, value) 
             VALUES (scheduled_time, measurement_time, table_name, table_schema, 
                     table_database, metric_name, metric_schema, argument_names, value);   
    END;
...
- Provider AWS Account: create a listing, attach the `ICEBERG_LAB` table and view objects, share with the Consumer AWS account.

Now run these queries and compare performance and results for Iceberg Tables with regular tables.
...
On <mark>AWS Consumer Account</mark> open a SQL Worksheet:
...
Try the same steps on <mark>Azure Consumer Account</mark>
...
### What we've covered
- How to blend Point-of-Sale and Marketplace Weather data to build analytics data products, then publish Listings targeted at accounts in your company 
- How to configure data privacy polices that are preserved in the consumer accounts that install the listings. Tag-based column masking, row-access, aggregation and projection policies will be created with database roles.
- How to setup Data Metrics Functions to monitor data quality of the shared data products
- How to share Iceberg Tables within a cloud region
- How to share unstructured text files and process with Cortex LLM functions
