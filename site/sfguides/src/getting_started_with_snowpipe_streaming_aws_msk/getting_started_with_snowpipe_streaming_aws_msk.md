...
First, in the Snowflake worksheet, replace `<pubKey>` with the content of the file `/home/ssm-user/pub.Key` (see `step 4` in `section #2` in the left pane) in the following SQL command and execute.
```commandline
use role accountadmin;
...
Start a new Linux session by following `step 3` in `section #2` in the left pane).
```commandline
