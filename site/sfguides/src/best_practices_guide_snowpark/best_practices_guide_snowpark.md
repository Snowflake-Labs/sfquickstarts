...
- Python 3.8
- Jupyter Notebook (or other notebook-based IDE that can open `.ipynb` files)
...
```bash
git clone https://github.com/Snowflake-Labs/TBD
cd TBD
```
...
Open up the jupyter notebook titled [lab1_cachetools_library](lab1_cachetools_library.ipynb) and run each of the cells.
...
Open up the jupyter notebook titled [lab2_vectorized_udfs](lab2_vectorized_udfs.ipynb) and run each of the cells.
...
Open up the jupyter notebook titled [lab3_avoid_pandas_df](lab3_avoid_pandas_df.ipynb) and run each of the cells.
