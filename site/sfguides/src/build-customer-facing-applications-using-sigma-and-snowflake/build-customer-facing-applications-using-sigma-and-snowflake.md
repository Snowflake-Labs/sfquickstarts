...
id: build-customer-facing-applications-using-sigma-and-snowflake
...
language: en
summary: Build customer-facing analytics applications with Sigma and Snowflake for embedded dashboards and self-service data access.
environments: web
status: Published 
...
2. `USE ROLE ACCOUNTADMIN;` This will set the context of the worksheet to use the role of ACCOUNTADMIN when we run the commands. This role holds the highest level of permission in a given Snowflake account, and can create new objects, grant permissions to other roles, and more. 
...
4. `USE DATABASE PC_SIGMA_DB;` This command tells Snowflake to operate off the PC_SIGMA_DB database, which was created when your Sigma trial was spun up.

5. `CREATE SCHEMA IF NOT EXISTS EMBEDDED_LAB;` This creates a new schema in our PC_SIGMA_DB database.

6. `USE SCHEMA EMBEDDED_LAB;` This sets the context of the worksheet to use our newly created schema.

7. `CREATE STAGE IF NOT EXISTS SIGMA_LAB_STAGE URL = 's3://sigma-embedded-lab-demo/LabData/';` This creates an external stage in Snowflake that points to an S3 bucket that has the data files we would like to use for the lab.
...
1. Run the following command to create a file format:
...
2. Run the following command to create the TRANSACTIONS table:
...
3. Run the following command:
   ...
   ```sql
   COPY INTO TRANSACTIONS FROM @SIGMA_LAB_STAGE
...
8. `SELECT * FROM TRANSACTIONS LIMIT 1000;`
...
8. Now we can install the needed libraries by issuing the following command: `npm install express uuid`.
...
6. Once complete, we are ready to start our server. Back in your terminal, you can run the following command to start the server: `node server.js`
...
7. In the formula bar, type `CountDistinct([Order Number])`. Rename this column Number of Orders. 

8. Again, click the drop down next to Order Channel and select "Add Column". This time, type `Sum([Revenue])` in the formula bar. What we can now see is Revenue generated for a specific region by the Order Channel the purchase was made from. 
