...
「**ワークスペース**」タブ（未移行の一部アカウントでは「ワークシート」）には、SQLクエリの送信、DDLおよびDML操作の実行、クエリまたは操作の完了時の結果表示のためのインターフェイスが用意されています。左上の「**+ Add new**」から、今回は`Notebook`を作成します。

![「ワークスペース」タブの詳細](assets/3UIStory_4.png)

左上には次の項目があります。

- **snowflake**アイコン：メインコンソールに戻ります。
- **ホーム**ボタン：メインコンソールに戻ります。
- **Create**ボタン：各種リソースの作成を行います。
- **Search**ボタン：ツールなどの検索を行う画面に移動します。


左サイドバーには次の項目があります。

- **Work with data**セクション：ワークスペースやダッシュボードなどのデータを取り扱う機能がまとめられています。
- **Horizon Catalog**セクション：データエクスプローラーなどのデータに関する機能がまとめられています。
- **Manage**セクション：ウェアハウスや権限などの管理に関する機能がまとめられています。
...
「**ワークスペース**」タブに移動します。[ワークスペース](#ワークスペース)で作成したノートブックが表示されます。
...
> 
> **コマンド実行のための多数のオプション。**UI、「ワークスペース」タブ、またはSnowSQLコマンドラインツールを使用して実行できます。また、お好みのSQLエディターを使用してODBC/JDBC経由で実行することも、その他のコネクタ（Python、Sparkなど）を使用して実行することもできます。前述のとおり、このラボでは、時間を節約するため、ほとんどの操作はUIを使用せずにワークシートで記述済みSQLを実行することで行われます。
...
では`citibike_trips`ステージのコンテンツを見ていきましょう。「**ワークスペース**」タブに戻り、作成した「CITIBIKE_ZERO_TO_SNOWFLAKE」ワークシートを開いて、下図のように、先ほどのコードの下に以下のSQLステートメントを追加して実行します。
...
「**ワークスペース**」タブで`CITIBIKE_ZERO_TO_SNOWFLAKE`ワークシートに戻ります。ワークシートのコンテキストが以下のように正しく設定されていることを確認してください。
