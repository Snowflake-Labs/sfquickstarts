    ...
    try:
        session.sql(f"""
            ...
            WHEN MATCHED THEN UPDATE SET
                CLI_DAILY_LIMIT = {cli_limit}, SNOWSIGHT_DAILY_LIMIT = {ss_limit},
                UPDATED_BY = '{safe_actor}', UPDATED_AT = CURRENT_TIMESTAMP(), IS_ACTIVE = TRUE
            WHEN NOT MATCHED THEN INSERT
                (CONFIG_TYPE, ROLE_NAME, CLI_DAILY_LIMIT, SNOWSIGHT_DAILY_LIMIT, CREATED_BY, UPDATED_BY, IS_ACTIVE)
            VALUES ('COHORT', '{safe_cohort}', {cli_limit}, {ss_limit}, '{safe_actor}', '{safe_actor}', TRUE)
        """).collect()
...
def _save_user_config(session, username, cli_val, ss_val, is_temporary=False, expires_at=None):
    ...
    try:
        session.sql(f"""
            ...
            WHEN MATCHED THEN UPDATE SET
                CLI_DAILY_LIMIT = {cli_val}, SNOWSIGHT_DAILY_LIMIT = {ss_val},
                UPDATED_BY = '{actor}', UPDATED_AT = CURRENT_TIMESTAMP(), IS_ACTIVE = TRUE
            WHEN NOT MATCHED THEN INSERT
                (CONFIG_TYPE, USER_NAME, CLI_DAILY_LIMIT, SNOWSIGHT_DAILY_LIMIT, CREATED_BY, UPDATED_BY, IS_ACTIVE)
            VALUES ('USER_OVERRIDE', '{safe_user}', {cli_val}, {ss_val}, '{actor}', '{actor}', TRUE)
        """).collect()
    except Exception as e:
        st.error(f"✗ Config save failed: {e}")
