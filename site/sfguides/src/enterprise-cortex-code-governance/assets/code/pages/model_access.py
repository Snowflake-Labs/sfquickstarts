...
def _render_tier_management(session):
    ...
    for tier_name, tier_info in tiers.items():
        _render_tier_card(session, tier_name, tier_info, tier_models.get(tier_name, []),
                          all_models, tiers, assignments, mdf, actor)


def _render_tier_card(session, tier_name, tier_info, models_in_tier,
                      all_models, all_tiers, assignments, mdf, actor):
    """Render a single tier card with inline edit and model management."""
    credits_in_tier = 0.0
    if not mdf.empty and "MODEL_NAME" in mdf.columns:
        for m in models_in_tier:
            row = mdf[mdf["MODEL_NAME"] == m]
            if not row.empty:
                credits_in_tier += float(row["CREDITS"].iloc[0])

    with st.container(border=True):
        # Header row
        col_title, col_credits, col_actions = st.columns([3, 1, 1])
        with col_title:
            st.markdown(f"### {tier_name}")
            st.caption(tier_info.get("description", ""))
        with col_credits:
            st.metric("30d Credits", f"{credits_in_tier:,.0f}",
                      help=f"Total credits consumed by all models in {tier_name} over the last 30 days.")
        with col_actions:
...
def _render_edit_tier_form(session, tier_name, tier_info, current_models,
    ...
    # ── Model assignment ───────────────────────────────────────────────────
    st.markdown("**Assign / Remove Models**")
    st.caption("Each model card shows its description and 30-day usage. Click to toggle.")
    ...
    for model in sorted(all_models):
        ...
        credits_used = 0.0
        if not mdf.empty and "MODEL_NAME" in mdf.columns:
            row = mdf[mdf["MODEL_NAME"] == model]
            if not row.empty:
                credits_used = float(row["CREDITS"].iloc[0])
        ...
        with c_card:
            ...
            st.markdown(
                ...
                + f'<div style="font-size:0.72rem;color:#58a6ff;margin-top:0.1rem;">30d credits: {credits_used:,.1f}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        with c_btn:
...
def _render_models(session):
    ...
    mdf = get_model_breakdown(session, 30, None)
    if not mdf.empty:
        st.markdown("**Credits by Model (Last 30 Days)**")
        mdf_sorted = mdf.sort_values("CREDITS", ascending=False)
        chart = alt.Chart(mdf_sorted).mark_bar().encode(
            x=alt.X('CREDITS:Q', title='Credits'),
            y=alt.Y('MODEL_NAME:N', sort=alt.EncodingSortField(field='CREDITS', order='descending'), title=''),
        ).properties(height=max(150, len(mdf_sorted) * 35)).configure_view(strokeWidth=0).configure(background='#0e1117')
    ...
    for model in discovered:
        ...
        credits_used = 0.0
        if not mdf.empty and "MODEL_NAME" in mdf.columns:
            match = mdf[mdf["MODEL_NAME"] == model]
            if not match.empty:
                credits_used = float(match["CREDITS"].iloc[0])
        ...
        with st.container(border=True):
            ...
            with col2:
                st.metric("30d Credits", f"{credits_used:,.1f}",
                          help=f"Credits consumed by {model} across all users in the last 30 days.")
...
def _render_mapping(session):
    ...
    enforcement = st.radio(
        "Enforcement method",
        [
            "Apply to Role Members (ALTER USER for each member)",
            "Apply to Account (affects ALL users — use with caution)",
            "Save mapping only (enforce later via scheduled task)",
        ],
        index=2, key="enforcement_method",
        help="Role Members = per-user allowlist. Account = blanket. Save only = deferred."
    )
    ...
    if st.button("Save & Apply", type="primary", key="btn_save_model_map",
        ...
        if "Role Members" in enforcement:
            _enforce_to_role_members(session, chosen_role, selected_models)
...
def _enforce_to_role_members(session, role_name, models):
    from utils import get_role_members
    members = get_role_members(session, role_name)
    if not members:
        st.warning(f"No direct members found in {role_name}. Allowlist not applied.")
        return
    ...
    for i, user in enumerate(members):
        try:
            safe_user = escape_sql_literal(user).upper()
            session.sql(f'ALTER USER "{safe_user}" SET CORTEX_MODELS_ALLOWLIST = \'{model_list}\'').collect()
            successes += 1
        except Exception:
            failures += 1
    ...
    log_activity(session, "ENFORCE_MODEL_ACCESS", target_role=role_name,
                 details={"method": "ROLE_MEMBERS", "models": models,
                          "members": len(members), "successes": successes, "failures": failures})
    if failures == 0:
        st.success(f"✓ Applied allowlist to all {successes} members of {role_name}")
    else:
        st.warning(f"Applied to {successes}, {failures} failed.")
...
def _enforce_to_account(session, models):
    ...
    try:
        ...
        st.error(f"✗ Failed (requires ACCOUNTADMIN): {e}")
