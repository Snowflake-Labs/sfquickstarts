...
# %% [markdown]
# ## Step 4: Model Training — XGBoost on Snowflake Compute
#
# Native XGBoost with Container Runtime. For this demo we train locally to keep
# it fast; in production you would use **distributed training on GPUs** via
# Snowpark Container Services. The model uses 6 engagement features to predict
...
# %% [markdown]
# ### ML Observability — Monitor Model Versions in Production
#
# Every model version stores **metrics, aliases, and deployment state** as
# first-class metadata. Query `SHOW VERSIONS` to compare AUC/F1 across the
# full version history, track which version carries the `DEFAULT` and
# `PRODUCTION` aliases, and see which versions are serving traffic via SPCS.

# %% --- Cell 14d: ML Observability ---
...
Observability patterns:
  • AUC improvement: V1 (0.50) → V3 (0.67) → V4 (0.68) — iterative gains
  • Alias routing: DEFAULT=V3 (batch), PRODUCTION=V4 (promoted)
  • ACTIVE_SERVICES > 0 means version is live on SPCS
  • Schedule SHOW VERSIONS in a Task for automated drift alerting
""")


# %% --- Cell 15: Model Explainability ---
test_sp_df = session.create_dataframe(X_test.head(5))

print("Testing built-in SHAP explainability...")
try:
    explanations = mv.run(test_sp_df, function_name="explain")
    print("SHAP Feature Attributions:")
    explanations.show()
except Exception as e:
    print(f"Note: {e}")
    print("""
The Snowflake inference environment currently includes SHAP 0.48.0 and
XGBoost 3.2.0, which have a known incompatibility. This affects the
built-in EXPLAIN function for XGBoost models.

Workaround: Use V2's PREDICT_WITH_FEATURES endpoint (Extension 1 below),
which returns per-feature contributions using XGBoost's native method.
""")
...
results = features_df.select("CREATOR_ID").join(
    ...
    "CREATOR_ID",
    scored['"output_feature_0"'].alias("P_NO_MATCH"),
    scored['"output_feature_1"'].alias("P_MATCH"),
).order_by('"P_MATCH"', ascending=False)
...
print("Invoking model directly via SQL (uses default version V3):")
session.sql("""
    SELECT
...
# %% --- Cell 22: REST API Examples ---
print("=" * 70)
print("REST API — Python Example")
print("=" * 70)
print("""
...
# contributions (the EXPLAIN workaround). This is the "when you need control" path.
...
class CreatorMatchMultiEndpoint(custom_model.CustomModel):
    ...
    @custom_model.inference_api
    def predict_ranked(self, input_df: pd.DataFrame, *, top_k: int = 10) -> pd.DataFrame:
        proba = self.model.predict_proba(input_df)[:, 1]
        result = input_df.copy()
        result["MATCH_SCORE"] = proba
        return result.sort_values("MATCH_SCORE", ascending=False).head(top_k).reset_index(drop=True)
...
print("\n--- predict_ranked (top_k=3) ---")
print(multi_model.predict_ranked(test_df, top_k=3).to_string(index=False))
...
WITH MV AS MODEL CC_DEMO.ML_REGISTRY.CREATOR_BRAND_MATCH VERSION V2
SELECT MV!PREDICT_RANKED(10, 0.05, 3, 150.0, 5, 0.7) AS RANKED;
...
# Note: CREATOR_PROFILES_LIVE is created in the CDP Extension (Cell 31).
# This cell is a placeholder — the full implementation is in Extension 6.
print("CREATOR_PROFILES_LIVE preview — see CDP Extension (Cell 31) for full implementation.")
print("The Dynamic Table auto-refreshes by joining creators + features + model scores.")
