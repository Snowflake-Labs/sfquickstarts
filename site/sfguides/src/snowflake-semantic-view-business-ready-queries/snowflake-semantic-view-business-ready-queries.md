...
To systematically explore how Semantic Views simplify queries, we organize examples using a **Question/Schema Complexity Matrix**. This framework helps you understand where Semantic Views provide the most value:

| Complexity Level | Question Complexity | Schema Complexity | What It Means |
|-----------------|---------------------|-------------------|---------------|
| **Low / Low** | Simple filters | 1-2 tables | Basic `SELECT` with `WHERE` clauses |
| **High / Low** | Aggregations, metrics | 1-2 tables | `GROUP BY`, `SUM()`, `COUNT()` on simple schemas |
| **Low / High** | Simple filters | 3+ tables with `JOIN`s | Easy question, but complex table relationships |
| **High / High** | Aggregations or derived columns | 3+ tables with `JOIN`s | Complex calculations across many tables |
...
**Semantic SQL:** Uses `FACTS` to retrieve customer attributes. The semantic view handles the `JOIN` between `customer` and `customer_demographics` automatically—no explicit `JOIN` needed.
...
**Semantic SQL:** Uses `METRICS` to call the pre-defined `total_sales` calculation. No need to write the complex `SUM(CAST(...))` formula—it's already defined in the semantic view. The `JOIN` to `date_dim` is automatic.
...
> **Note:** Query 22 contains a complex `CASE` statement for derived columns, making it a hybrid that could also be classified as "High Question / High Schema." It's included here to show how Semantic Views can pre-define complex calculations once and reuse them.

### Query 21: Customers in Income Band 9
...
### Query 22: Net Profit Tier by Store and Gender
...
**Semantic SQL:** Uses `FACTS` with the pre-defined `f_net_profit_tier` calculation. The complex `CASE` statement is encapsulated in the semantic view—no need to rewrite the tiering logic. `JOIN`s across 4 tables are automatic.
...
### Query 23: Customers at Store 'ese' in 2001
...
### Query 32: Store Metrics by State in 2002
...
### Query 33: Store Sales by Manager in Tennessee
...
### Query 34: Web Sales by Site in New Jersey
