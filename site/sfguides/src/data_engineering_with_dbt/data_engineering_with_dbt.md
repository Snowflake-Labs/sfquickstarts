...
[dbt](https://www.getdbt.com/) is a popular data transformation tool that uses software engineering best practices like modularity, portability, testing, and version control to enable data teams to ship trusted data sets, faster. dbt transforms data in the warehouse, leveraging cloud data platforms like Snowflake.
...
* A [dbt Cloud account](https://www.getdbt.com/signup/) installed 
...
2. Sign in to continue. You will use the same user and password that you used to login to your Snowflake account the first time. 
...
7. In the pop-up, enter “KNOEMA_ECONOMY_DATA_ATLAS” as the database name and click “Get Data”. Do not forget to do this or you will have change some code down the line.
...
12. Now we can see what our datasets names are. 
Let's try to query one of the datasets:
...
2. Find the dbt Tile either by typing into the "Search Partner Connect" search bar. Click on the tile. 

![Click on dbt Partner Connect](assets/click_on_dbt_Partner_Connect.png)

3. You should now see a popup that says “Connect to dbt”. Click on Optional Grant and you are going to add in the KNOEMA_ECONOMY_DATA_ATLAS database. This will grant access for your new dbt user role to the database. Click on Connect. This will create a dedicated dbt user, database, warehouse, and role for your dbt Cloud trial.  

![Click on Connect](assets/click_on_dbt_Connect.png)

4. When the popup that says "Your partner account has been created", click on activate. 
![Click on Partner Activate](assets/click_on_dbt_connect_activate.png)

5. You should be redirected to a dbt Cloud registration page. Fill out the form. 
...
6. Click on Compete Registration. You should now be redirected to your dbt Cloud account, complete with a connection to your Snowflake account, a deployment and a development environment, and even a sample job. 
...
6. UI walkthrough (SA will walk through during the workshop). Post workshop, users can look at [this video for the walkthrough](https://getdbt.wistia.com/medias/0828jynq3v).
7. Click “create new branch” to check out a new git branch to start developing. 
![create branch](assets/create_branch.png)

8. Name the branch “dbt_snowflake_workshop” and click “Submit”.
...
17. Next, let’s update the [dbt_project.yml file](https://docs.getdbt.com/reference/dbt_project.yml). This is a file that tells dbt that the repository is a dbt project as well as how to operate on your project. Click “dbt_project.yml”. The first thing we are going to do is update the project name from “my_new_project” to “<your_name>_dbt_workshop” at line 5. This will personalize the project a little, and make it yours. You will want to update the models section below with the same project name. Do not forget to save after you have updated the file! This means you will want to either click save or use cmd+s (mac)/ctrl+s(pc). 

![update project](assets/update_project.png)


18. Now let’s update the models section of the [dbt_project.yml](https://docs.getdbt.com/reference/dbt_project.yml) file by declaring our new folders and their model configurations. We will be declaring the default [schema](https://docs.getdbt.com/docs/building-a-dbt-project/building-models/using-custom-schemas) that dbt builds the object into, the default [materialization](https://docs.getdbt.com/docs/building-a-dbt-project/building-models/materializations), and disabling any unnecessary models from a run. All of these configurations can be overridden by model configurations on the model file. If a schema is not declared via a configuration, dbt will build into the default schema declared on our development credentials. 
...
In this workshop, we will show you how different environment configurations can easily change where your dbt project reads and writes from without needing you to change your code with code promotion. 


Macros are a great way to do that so let’s talk more about it. Macros are written in a pythonic templating language called [Jinja](https://docs.getdbt.com/docs/building-a-dbt-project/jinja-macros). Jinja allows you to do things that normally aren’t possible in SQL such as create control structures (like if statements and for loops) or abstract away snippets of SQL into reusable functions applicable throughout your project. Jinja helps you write [DRY](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself) code in a language that is generally wet. Macros are those reusable functions. 
...
30. Now we are going to install a dbt package. A dbt [package](https://docs.getdbt.com/docs/building-a-dbt-project/package-management) is essentially a dbt project that you can install onto your own dbt project to gain access to the code and use it as your own. Many of our packages are hosted on the [dbt Packages Hub](https://hub.getdbt.com/). In our lab, we are going to demonstrate how to use some useful macros that the dbt_utils package provides to write some complex SQL. To install it, create a file called packages.yml at the same level as your dbt_project.yml file.
...
32. Click “Save”. 
33. Now we are going to install the package. By running “dbt deps” you tell dbt to install its packages or dependencies to help you remember the command. With a successful run, you can check out the dbt_modules folder to see what packages you have installed and the code that is now available to you. 
![packages](assets/packages_3.png)

34. Last thing to do before we save our work. Delete the example folder and the models in it since we will now be building our own. 
...
As you probably remember, these two objects were mentioned in the Knoema Dataset Catalog table: daily exchange rates and daily US trading history accordingly.

3. Now let’s save our work and commit. Click on “Commit” and provide a good commit message like “set up knoema source file”. We are going to keep building that muscle of saving your work as you develop. 
...
6. Save the file. To check the query before executing the code against the warehouse (and thus executing it in Snowflake), you can click on the “Preview” button. You can also click on “Compile” to see the compiled code that will be executed. 
...
- This also allows dbt to dynamically replace the sources with their target-specific name. If the source table ever moves, you can update it on the sources.yml file and it will carry downstream to any models referenced to that source without manually interfering.  
...
This allows us to be selective and only run the models in the staging folder. To find out more about other node selectors that you can use, [check out this doc](https://docs.getdbt.com/reference/node-selection/syntax).
...
You should see that we have run all of the models (excluding sources) to the left of this lineage. 
...
dbt is able to generate a static webpage with a data dictionary/documentation by pulling in information from your dbt project as well as your Snowflake information_schema. It also provides an interactive DAG so you can see the full lineage of your models; this is something you might have also noticed  in the lineage tab of the IDE. This is a fantastic way to share  information with your internal teams, as it contains all-important information about columns, tags, free-form model description, 

tests as well as the source code that is always in line with the code. So regardless of how big your project grows, it is super easy to understand what's happening with the help of dbt’s documentation. 
