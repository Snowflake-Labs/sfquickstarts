...
**Version:** 1.0.0 | **Author:** RC | **Date:** May 14, 2026
...
│   ├── usage_trends.py           Usage dashboards + charts (admin)
│   ├── credit_requests.py        Request form (user) + approval queue (admin)
│   ├── settings.py               App configuration + AI Budgets (admin)
│   ├── audit_logs.py             Audit log viewer (admin)
│   ├── observability.py, model_intelligence.py, model_access.py, model_bakeoff.py,
│   │                             user_intelligence.py, prompt_analysis.py, policy_rules.py,
│   │                             alerts.py, user_quotas.py, setup.py   (governance/admin)
│   └── chargeback.py, attribution.py, warehouse_activity.py             (chargeback)
