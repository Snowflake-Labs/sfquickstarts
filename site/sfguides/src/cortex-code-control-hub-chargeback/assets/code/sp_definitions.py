...
def handler(session, lookback_hours):
    ...
    try:
        wm_df = session.sql(f"""
            SELECT TO_VARCHAR(
                     COALESCE(DATEADD('minute',-120,MAX(EVENT_TS)),DATEADD('day',-30,CURRENT_TIMESTAMP())),
                     'YYYY-MM-DD HH24:MI:SS.FF3') AS WM
        ...
        new_df = session.sql(f"""
            SELECT DATE_TRUNC('day',e.TIMESTAMP)::DATE AS EVENT_DATE,
                   TO_VARCHAR(e.TIMESTAMP::TIMESTAMP_NTZ,'YYYY-MM-DD HH24:MI:SS.FF3') AS EVENT_TS,
        ...
        if not new_df.empty:
            ...
            try:
                ...
                # EVENT_TS arrives as a formatted string 'YYYY-MM-DD HH24:MI:SS.FFF';
                # keep it as a string so write_pandas -> COPY casts it into the
                # TIMESTAMP_NTZ column. Converting to a pandas nanosecond datetime
                # here corrupts the stored value (scale mismatch on write).
                clean['EVENT_TS']   = clean['EVENT_TS'].astype(str)
            except Exception:
                pass
...
def handler(session, mode):
    ...
    def _email_html(name, atype, msg, account, timestamp):
        ...
        return (
            ...
            '<span style="color:#7dd3fc;font-size:18px;font-weight:700">CoCo Control Hub &amp; Chargeback</span>'
            ...
            'Sent by <strong style="color:#64748b">CoCo Control Hub &amp; Chargeback</strong> &#160;&#183;&#160; '
