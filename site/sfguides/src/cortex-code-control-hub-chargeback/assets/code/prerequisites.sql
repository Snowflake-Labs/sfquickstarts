...
BEGIN
    ...
    -- Store metadata
    SELECT COALESCE(CURRENT_USER(), 'SYSTEM') INTO v_actor;
...
EXCEPTION WHEN OTHER THEN
    v_err := SQLERRM;
    INSERT INTO CC_AUDIT_LOG (ACTOR, ACTION_TYPE, DETAILS, STATUS)
    SELECT COALESCE(CURRENT_USER(), 'SYSTEM'), 'CREATE_AI_BUDGET',
...
BEGIN
    ...
    USING (
        SELECT
            DATE_TRUNC('day', h.START_TIME)::DATE AS USAGE_DATE,
            h.USERNAME AS USER_NAME,
            MAX(uc.COHORT_ROLE) AS COHORT_ROLE,
            'ANALYST' AS SURFACE,
            'UNKNOWN' AS MODEL_NAME,
            SUM(h.CREDITS) AS TOTAL_CREDITS,
            0 AS TOTAL_TOKENS,
            SUM(h.REQUEST_COUNT) AS QUERY_COUNT
        FROM SNOWFLAKE.ACCOUNT_USAGE.CORTEX_ANALYST_USAGE_HISTORY h
        LEFT JOIN CC_TEMP_USER_COHORT uc ON uc.USER_NAME = h.USERNAME
        WHERE h.START_TIME > :v_last_refresh
        GROUP BY 1, 2, 4, 5
    ) src
    ON tgt.USAGE_DATE = src.USAGE_DATE
        AND tgt.USER_NAME = src.USER_NAME
    ...
    -- HOURLY SUMMARY: ANALYST
    MERGE INTO CC_USAGE_HOURLY_SUMMARY tgt
    USING (
        SELECT
            DATE_TRUNC('day', h.START_TIME)::DATE AS USAGE_DATE,
            EXTRACT(HOUR FROM h.START_TIME)::NUMBER(2,0) AS USAGE_HOUR,
            h.USERNAME AS USER_NAME,
            MAX(uc.COHORT_ROLE) AS COHORT_ROLE,
            'ANALYST' AS SURFACE,
            SUM(h.CREDITS) AS TOTAL_CREDITS,
...
-- 9. CHARGEBACK TIER 1 — warehouse-credit attribution + waterfall
--    (full standalone script also shipped as chargeback_tier1.sql)
--    Adds the compute half of CoCo cost: the warehouse credits CoCo burns
--    running SQL (LLM tokens are already in CC_USAGE_DAILY_SUMMARY), plus a
--    trimmed confidence-labeled waterfall and an unattributed queue.
-- ---------------------------------------------------------------------------
-- ---------------------------------------------------------------------------
-- Cost-tag mapping (USER -> Vertical / Partner-flag). Enables group-by
-- Vertical / Partner-flag in the Chargeback bill flow and is read by
-- SP_CC_ATTRIBUTE_USAGE / written by SP_CC_SYNC_COST_TAGS. App writes it
-- directly (owner-rights as ACCOUNTADMIN), so NO ownership transfer to SP_OWNER.
CREATE TABLE IF NOT EXISTS CC_COST_TAGS (
    USER_NAME   VARCHAR(255) NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS CC_ATTRIBUTION_DAILY (
    ...
    ATTR_METHOD       VARCHAR(40)   NOT NULL,
...
-- ---------------------------------------------------------------------------
-- 9b. COST-TAG SYNC  (read-only: ACCOUNT_USAGE.TAG_REFERENCES -> CC_COST_TAGS)
-- ---------------------------------------------------------------------------
-- Snapshots Snowflake object tags assigned to USERS into the app-owned
-- CC_COST_TAGS so the Chargeback group-by dimensions populate automatically.
-- STRICTLY READ-ONLY on the account: it never runs ALTER USER / SET TAG and
-- writes nothing outside this schema. Prescribed tag keys (case-insensitive):
--   VERTICAL   <- first of: VERTICAL, COST_CENTER, TEAM, BUSINESS_UNIT
--   IS_PARTNER <- COCO_PARTNER / IS_PARTNER / PARTNER with a truthy value
-- Non-destructive MERGE: fills blanks and is partner-flag sticky; it never
-- erases a manually-entered tag. Retrofit is automatic (TAG_REFERENCES reflects
-- current assignments; billing is keyed at the user level from day 0).
CREATE OR REPLACE PROCEDURE SP_CC_SYNC_COST_TAGS()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS
...
DECLARE
    v_rows NUMBER DEFAULT 0;
BEGIN
    MERGE INTO CC_COST_TAGS t
    USING (
        SELECT * FROM (
            SELECT
                OBJECT_NAME AS USER_NAME,
                COALESCE(
                    MAX(CASE WHEN UPPER(TAG_NAME) = 'VERTICAL'    THEN TAG_VALUE END),
                    MAX(CASE WHEN UPPER(TAG_NAME) = 'COST_CENTER' THEN TAG_VALUE END),
                    MAX(CASE WHEN UPPER(TAG_NAME) IN ('TEAM','BUSINESS_UNIT') THEN TAG_VALUE END)
                ) AS VERTICAL,
                BOOLOR_AGG(
                    UPPER(TAG_NAME) IN ('COCO_PARTNER','IS_PARTNER','PARTNER')
                    AND UPPER(NVL(TAG_VALUE,'')) IN ('TRUE','YES','Y','1','PARTNER')
                ) AS IS_PARTNER
        ...
        WHERE VERTICAL IS NOT NULL OR IS_PARTNER
    ) s
    ON t.USER_NAME = s.USER_NAME
    WHEN MATCHED THEN UPDATE SET
        VERTICAL   = COALESCE(s.VERTICAL, t.VERTICAL),
        IS_PARTNER = (COALESCE(t.IS_PARTNER, FALSE) OR s.IS_PARTNER),
        UPDATED_BY = 'SP_CC_SYNC_COST_TAGS',
        UPDATED_AT = CURRENT_TIMESTAMP()
    WHEN NOT MATCHED THEN INSERT
        (USER_NAME, VERTICAL, IS_PARTNER, UPDATED_BY, UPDATED_AT)
        VALUES (s.USER_NAME, s.VERTICAL, s.IS_PARTNER,
                'SP_CC_SYNC_COST_TAGS', CURRENT_TIMESTAMP());

    SELECT COUNT(DISTINCT OBJECT_NAME) INTO v_rows
      FROM SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES
      WHERE DOMAIN = 'USER' AND OBJECT_DELETED IS NULL
        AND UPPER(TAG_NAME) IN
            ('VERTICAL','COST_CENTER','TEAM','BUSINESS_UNIT',
             'COCO_PARTNER','IS_PARTNER','PARTNER');

    RETURN 'OK: synced cost tags from ACCOUNT_USAGE.TAG_REFERENCES ('
           || v_rows || ' tagged user(s) found).';
END;
$$;

CREATE OR REPLACE TASK CC_SYNC_COST_TAGS
    WAREHOUSE = __WH__
    SCHEDULE  = 'USING CRON 0 4 * * * UTC'
    COMMENT   = 'Daily: snapshot user object tags into CC_COST_TAGS (read-only)'
AS
    CALL SP_CC_SYNC_COST_TAGS();

ALTER TASK CC_SYNC_COST_TAGS RESUME;

GRANT USAGE ON PROCEDURE SP_CC_SYNC_COST_TAGS() TO ROLE __APP_ROLE__;
