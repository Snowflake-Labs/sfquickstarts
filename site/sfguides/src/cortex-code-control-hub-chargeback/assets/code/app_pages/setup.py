...
def render(session):
    ...
    st.header("Setup", help="One-time installation and data bootstrap for CoCo Control Hub & Chargeback.")
...
def _run_setup(session):
    ...
    db = get_app_database(session)
    schema = get_app_schema(session)
    # Default to the session's current warehouse (the one running this app);
    # resolved live below so nothing account-specific is hardcoded.
    warehouse = None
    ...
    if not warehouse:
        st.error("Could not determine the current warehouse. Select a warehouse "
                 "for this session (USE WAREHOUSE ...) and re-run Setup.")
        return
