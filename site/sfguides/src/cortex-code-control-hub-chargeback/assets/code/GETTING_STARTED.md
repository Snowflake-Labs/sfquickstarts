# CoCo Control Hub & Chargeback — AI Deployment Starter Prompt

Copy and paste the text below into your AI assistant (Cortex Code, Claude, or any LLM).
Point it to the folder containing the app code.
The AI will ask you questions and guide you through the full deployment.

Prefer to deploy manually instead? See `DEPLOYMENT_GUIDE.md` for the step-by-step
instructions (`snow streamlit deploy` + the app's built-in Setup page).

---

## How to Use

1. Point your AI assistant at this folder (the app code)
2. Copy the prompt below
3. Paste it into your AI session
4. Answer the questions the AI asks — it will do the rest

If you are using the Cortex Code CLI, start a session with `cortex`, then paste the
starter prompt.
...
I want to deploy CoCo Control Hub & Chargeback (Cortex Code Credit Manager) in my Snowflake account.

The app code is in this folder. Please guide me through the deployment.

Before we start, here is what I need you to know:

1. **Do NOT modify any source code files.** The only files you may change are `snowflake.yml` and `config.yaml`. All other files — `sp_definitions.py`, `config.py`, `prerequisites.sql`, `streamlit_app.py`, and all files in `app_pages/` — must be deployed exactly as they are.
...
CoCo Control Hub & Chargeback is a governance dashboard for Cortex Code usage in Snowflake. It gives admins:
