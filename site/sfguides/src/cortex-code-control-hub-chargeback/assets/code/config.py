...
APP_NAME = "CoCo Control Hub & Chargeback"
APP_VERSION = "3.1.1"
APP_ICON = "⚡"
...
TABLE_COST_TAGS = "CC_COST_TAGS"
# ── Chargeback Tier 1 (warehouse-credit attribution + waterfall) ──────────────
TABLE_WAREHOUSE_USAGE_DAILY = "CC_WAREHOUSE_USAGE_DAILY"
TABLE_SERVICE_USER_MAPPING  = "CC_SERVICE_USER_MAPPING"
TABLE_ROLE_MAPPING          = "CC_ROLE_MAPPING"
TABLE_ATTRIBUTION_DAILY     = "CC_ATTRIBUTION_DAILY"
TABLE_UNATTRIBUTED          = "CC_UNATTRIBUTED"
# ── AI Budgets + Native Per-User Quotas ───────────────────────────────────────
TABLE_AI_BUDGETS      = "CC_AI_BUDGETS"
TABLE_AI_BUDGET_USAGE = "CC_AI_BUDGET_USAGE"
TABLE_NATIVE_QUOTAS   = "CC_NATIVE_QUOTAS"

# ── Chargeback attribution registry ──────────────────────────────────────────
# Drives the "Bill by" group-by knob. `source` tells the loader which base query
# to use:
#   usage         → CC_USAGE_DAILY_SUMMARY grouped by `column` (CoCo credits)
#   metering      → METERING_DAILY_HISTORY by SERVICE_TYPE (whole-account bill)
#   usage_tagged  → CC_USAGE_DAILY_SUMMARY LEFT JOIN CC_COST_TAGS, grouped by tag
# `gated` dimensions require CC_COST_TAGS to be populated before they light up.
ATTRIBUTION_DIMENSIONS = [
    {"key": "user",     "label": "By User",                 "source": "usage",        "column": "USER_NAME",    "gated": False},
    {"key": "cohort",   "label": "By Role / Cohort",        "source": "usage",        "column": "COHORT_ROLE",  "gated": False},
    {"key": "surface",  "label": "By Surface",              "source": "usage",        "column": "SURFACE",      "gated": False},
    {"key": "model",    "label": "By Model",                "source": "usage",        "column": "MODEL_NAME",   "gated": False},
    {"key": "service",  "label": "By Service (whole acct)", "source": "metering",     "column": "SERVICE_TYPE", "gated": False},
    {"key": "vertical", "label": "By Vertical",             "source": "usage_tagged", "column": "VERTICAL",     "gated": True},
    {"key": "partner",  "label": "By Partner-flag",         "source": "usage_tagged", "column": "IS_PARTNER",   "gated": True},
]
...
MODEL_PRESETS = {
    "M1": {"badge": "M1", "title": "Internal Cross-Charge",
           "tagline": "Bill teams inside one account",
           "subtitle": "one shared account; bill each internal business unit / vertical",
           "description": "One shared account; the platform owner allocates CoCo cost to each "
                          "internal business unit or vertical (IT, Finance, product teams). No "
                          "external party — works for a customer or a partner's own org.",
           "flow": "Platform owner  →  $ chargeback  →  internal business units",
           "dimension": "vertical", "audience": "internal"},
    "M2": {"badge": "M2", "title": "Build Here, Deploy There",
           "tagline": "Run in your account, deliver to another",
           "subtitle": "you run CoCo on your account, invoice the receiving customer",
           "description": "You run CoCo in your own account and deploy the finished work into "
                          "another account. CoCo spend lands on your side, so you invoice the "
                          "receiving customer for that usage.",
           "flow": "Your account (CoCo spend)  →  $ invoice  →  receiving customer",
           "dimension": "cohort", "audience": "external",
           "caveat": "True engagement billing needs per-session tags (Phase 2). "
                     "Grouping by role/cohort as an interim proxy."},
    "M3": {"badge": "M3", "title": "Partner on Customer Account",
           "tagline": "Isolate guest usage, bill it back",
           "subtitle": "guest staff use another org's account; the host bills them back",
           "description": "Your staff work inside another org's (the host's) account with their own "
                          "logins. The host isolates that CoCo usage and charges it back to you — the "
                          "classic partner-on-customer-account case.",
           "flow": "Host account  →  $ chargeback  →  guest org (e.g. partner)",
           "dimension": "user", "audience": "external"},
}


def get_dimension(dim_key: str) -> dict:
    """Return the ATTRIBUTION_DIMENSIONS entry for a key (defaults to 'user')."""
    for d in ATTRIBUTION_DIMENSIONS:
        if d["key"] == dim_key:
            return d
    return ATTRIBUTION_DIMENSIONS[0]
...
# Chargeback Tier 1 — warehouse-credit refresh + waterfall attribution
SP_REFRESH_WAREHOUSE_USAGE = "SP_CC_REFRESH_WAREHOUSE_USAGE"
SP_ATTRIBUTE_USAGE = "SP_CC_ATTRIBUTE_USAGE"
SP_SYNC_COST_TAGS = "SP_CC_SYNC_COST_TAGS"
# ── AI Budget + Quota SPs ─────────────────────────────────────────────────────
...
# Waterfall attribution levels (trimmed, confidence-labeled). Drives the
# Attribution page explainer. `order` = the exact precedence SP_CC_ATTRIBUTE_USAGE
# applies (first match wins, best signal first): service-user > user-tag > role;
# anything unmatched → CC_UNATTRIBUTED queue. `effort` = what the platform owner
# must set up — the more they set up, the more usage resolves at higher confidence.
WATERFALL_LEVELS = [
    {"order": 1, "method": "L3_SERVICE_USER", "signal": "The user is a mapped service / partner account",
     "you_set_up": "Map the service username → entity", "effort": "Medium", "confidence": "HIGH",
     "source": "CC_SERVICE_USER_MAPPING",
     "note": "Most reliable. Best for M2 (build & deploy) and M3 (partner on customer account)."},
    {"order": 2, "method": "L4_USER_TAG", "signal": "The user carries a vertical / partner tag",
     "you_set_up": "Tag users with a vertical / partner-flag", "effort": "Low", "confidence": "MEDIUM",
     "source": "CC_COST_TAGS",
     "note": "Quick win. Best for M1 (internal cross-charge by business unit)."},
    {"order": 3, "method": "L5_ROLE", "signal": "The user holds a mapped Snowflake role",
     "you_set_up": "Map a role → entity", "effort": "Low–Med", "confidence": "MEDIUM",
     "source": "CC_ROLE_MAPPING",
     "note": "Useful when a partner's staff share a role (M3)."},
    {"order": 4, "method": "UNATTRIBUTED", "signal": "No identity signal found",
     "you_set_up": "—", "effort": "—", "confidence": "NONE",
     "source": "→ Unattributed queue",
     "note": "Routed to the queue for review — never billed blindly."},
]
...
PAGE_CHARGEBACK = "Generate Bill"
PAGE_WAREHOUSE_ACTIVITY = "CoCo Warehouse Activity"
PAGE_ATTRIBUTION = "Attribution & Tags"
PAGE_MODEL_BAKEOFF = "Model Bake-off"
PAGE_NATIVE_QUOTAS = "Native Quotas"

# ── Sidebar sections ──────────────────────────────────────────────────────────
# The Chargeback capabilities render under their own heading in the sidebar
# to keep chargeback grouped and uncluttered.
CHARGEBACK_PAGES = [
    PAGE_CHARGEBACK,           # Generate Bill (tokens + warehouse compute)
    PAGE_WAREHOUSE_ACTIVITY,   # CoCo Warehouse Activity drill-down
    PAGE_ATTRIBUTION,          # Attribution & Tags (waterfall + unattributed queue)
]
...
ADMIN_PAGES = CORE_ADMIN_PAGES + CHARGEBACK_PAGES

USER_PAGES = [PAGE_HOME, PAGE_CREDIT_REQUESTS]
ALL_PAGES = [PAGE_HOME] + CORE_ADMIN_PAGES + [PAGE_CREDIT_REQUESTS] + CHARGEBACK_PAGES

GLOBAL_CSS = """
<style>
/* ── Hide auto-generated Streamlit native page nav (we use a custom st.radio nav) ── */
...
/* ── Layout ──────────────────────────────────────────────────────────────── */
.block-container { padding-top: 3.5rem !important; max-width: 1280px !important; }
hr { border-color: #1f2937 !important; margin: 0.8rem 0 !important; }

/* ── Chargeback model selector cards ─────────────────────────────────────── */
.cc-model { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.2rem; }
.cc-model-badge { font-size: 0.7rem; font-weight: 700; color: #9ca3af;
    border: 1px solid #374151; border-radius: 0.35rem; padding: 0.05rem 0.4rem; }
.cc-model-title { font-size: 1.15rem; font-weight: 700; color: #f9fafb; line-height: 1.2; }
.cc-model-sel .cc-model-title { color: #3b82f6; }
.cc-model-tag { font-size: 0.9rem; font-weight: 600; color: #7dd3fc; margin-bottom: 0.4rem; }
.cc-model-desc { font-size: 0.82rem; color: #c9d1d9; line-height: 1.4; margin-bottom: 0.5rem; }
.cc-flow { font-size: 0.82rem; font-weight: 600; color: #e5e7eb; background: #111827;
    border: 1px solid #1f2937; border-radius: 0.4rem; padding: 0.45rem 0.6rem; margin: 0.5rem 0; }

/* ── Attribution: end-to-end flow step cards ─────────────────────────────── */
.cc-step { background: #0f1620; border: 1px solid #1f2937; border-radius: 0.5rem;
    padding: 0.55rem 0.7rem; height: 100%; }
.cc-step-num { display: inline-flex; align-items: center; justify-content: center;
    width: 1.25rem; height: 1.25rem; border-radius: 50%; background: #1d4ed8; color: #fff;
    font-size: 0.72rem; font-weight: 700; margin-right: 0.4rem; }
.cc-step-title { font-size: 0.92rem; font-weight: 700; color: #f9fafb; }
.cc-step-desc { font-size: 0.78rem; color: #c9d1d9; line-height: 1.35; margin-top: 0.3rem; }

/* ── Attribution: waterfall ladder ───────────────────────────────────────── */
.cc-wf-row { display: flex; align-items: center; gap: 0.6rem; background: #111827;
    border: 1px solid #1f2937; border-left-width: 4px; border-radius: 0.4rem;
    padding: 0.5rem 0.7rem; margin-bottom: 0.4rem; }
.cc-wf-hi   { border-left-color: #6ee7b7; }
.cc-wf-med  { border-left-color: #fcd34d; }
.cc-wf-none { border-left-color: #f87171; }
.cc-wf-skip { border-left-color: #4b5563; opacity: 0.72; }
.cc-wf-badge { font-size: 0.72rem; font-weight: 700; color: #9ca3af; border: 1px solid #374151;
    border-radius: 0.35rem; padding: 0.05rem 0.4rem; white-space: nowrap; }
.cc-wf-main { flex: 1; }
.cc-wf-sig { font-size: 0.95rem; font-weight: 600; color: #f9fafb; }
.cc-wf-set { font-size: 0.82rem; color: #9ca3af; margin-top: 0.12rem; }
.cc-chip { font-size: 0.68rem; font-weight: 700; border-radius: 0.7rem; padding: 0.1rem 0.5rem;
    white-space: nowrap; }
.cc-chip-hi   { background: rgba(110,231,183,0.15); color: #6ee7b7; }
.cc-chip-med  { background: rgba(252,211,77,0.15);  color: #fcd34d; }
.cc-chip-none { background: rgba(248,113,113,0.15); color: #f87171; }
.cc-chip-skip { background: rgba(148,163,184,0.15); color: #94a3b8; }

/* ── Attribution: explainer callout ──────────────────────────────────────── */
.cc-callout { background: #0f1620; border: 1px solid #1f2937; border-left: 4px solid #29b5e8;
    border-radius: 0.4rem; padding: 0.6rem 0.8rem; margin: 0.3rem 0; }
.cc-callout .h { font-size: 0.9rem; font-weight: 700; color: #f9fafb; margin-bottom: 0.25rem; }
.cc-callout p { font-size: 0.8rem; color: #c9d1d9; line-height: 1.45; margin: 0.15rem 0; }
.cc-lever-title { font-size: 1.02rem; font-weight: 700; color: #f9fafb; }
.cc-lever-sub { font-size: 0.78rem; color: #9ca3af; margin-top: 0.15rem; margin-bottom: 0.2rem; }
