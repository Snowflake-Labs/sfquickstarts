...
categories: snowflake-site:taxonomy/solution-center/certification/quickstart, snowflake-site:taxonomy/product/ai, snowflake-site:taxonomy/product/applications-and-collaboration, snowflake-site:taxonomy/snowflake-feature/cortex-llm-functions
environments: web
status: Published
feedback link: https://github.com/Snowflake-Labs/sfguides/issues

# CoCo Control Hub and Chargeback Solution
...
![Attribution and billing flow: ACCOUNT_USAGE through the attribution waterfall to an itemized table, CSV, and branded PDF](assets/architecture_flow.png)
...
![Attribution waterfall: L3 service account (HIGH) to L4 user cost tag to L5 role (MEDIUM) to an Unattributed Queue that is never billed](assets/attribution_waterfall.png)
...
> **Important:** Per-query and per-session query tags do **not** work for CoCo — it overwrites the session query tag. Attribution uses identity-level signals (L3–L5), not L1/L2 tags.
...
> **Note:** Leave `config.yaml`'s deployment fields blank — the app self-resolves database/schema at runtime (sidebar override, then `config.yaml`, then `CURRENT_DATABASE()`/`CURRENT_SCHEMA()`). There is nothing else to configure.
...
> **Tip:** If your account already tags users natively, enable auto-sync — the nightly `SP_CC_SYNC_COST_TAGS` mirrors `ACCOUNT_USAGE.TAG_REFERENCES` into `CC_COST_TAGS` read-only. The app never modifies account objects.
...
> **Tip:** Toggle **Include SQL / warehouse cost** to add the warehouse compute CoCo consumed (priced at your contract rate) — the KPI and composition bar update to show the AI-vs-warehouse split.
...
![Sample branded invoice PDF generated by the app](assets/invoice_pdf.png)

> **Note:** Same data, two outputs: internal cross-charge is at-cost showback; external delivery is an invoice with margin. That's the "one configurable app" design — you changed audience, not code.
