...
def run_sql(sql: str, return_result: bool = False):
    try:
...
st.title(":material/database: Day 26: Agent Setup")
st.write("Set up the data infrastructure and create the Cortex Agent.")

with st.expander("Step 1: Create Database & Tables", expanded=True):
    st.markdown("Creates `SALES_INTELLIGENCE` database with conversation and metrics tables.")
    if st.button("Create Database & Tables", key="step1"):
        with st.spinner("Creating database and tables..."):
            sqls = [
                "CREATE OR REPLACE DATABASE SALES_INTELLIGENCE",
                "CREATE OR REPLACE SCHEMA SALES_INTELLIGENCE.DATA",
                """CREATE OR REPLACE TABLE SALES_INTELLIGENCE.DATA.SALES_CONVERSATIONS (
                    conversation_id VARCHAR, transcript_text TEXT, customer_name VARCHAR,
                    deal_stage VARCHAR, sales_rep VARCHAR, conversation_date TIMESTAMP,
                    deal_value FLOAT, product_line VARCHAR)""",
                """INSERT INTO SALES_INTELLIGENCE.DATA.SALES_CONVERSATIONS VALUES
                    ('CONV001', 'Initial discovery call with TechCorp Inc...', 'TechCorp Inc', 'Discovery', 'Sarah Johnson', '2024-01-15 10:30:00', 75000, 'Enterprise Suite'),
                    ('CONV002', 'Follow-up call with SmallBiz Solutions...', 'SmallBiz Solutions', 'Negotiation', 'Mike Chen', '2024-01-16 14:45:00', 25000, 'Basic Package')""",
                """CREATE OR REPLACE TABLE SALES_INTELLIGENCE.DATA.SALES_METRICS (
            ...
            for sql in sqls:
                if not run_sql(sql):
                    st.stop()
            st.success("Database and tables created!")

with st.expander("Step 2: Create Cortex Search Service"):
    st.markdown("Creates search service for conversation transcripts.")
    if st.button("Create Search Service", key="step2"):
        with st.spinner("Creating search service..."):
            run_sql("ALTER TABLE SALES_INTELLIGENCE.DATA.SALES_CONVERSATIONS SET CHANGE_TRACKING = TRUE")
            sql = """CREATE CORTEX SEARCH SERVICE IF NOT EXISTS SALES_INTELLIGENCE.DATA.SALES_CONVERSATION_SEARCH
                ON transcript_text ATTRIBUTES customer_name, deal_stage, sales_rep
                WAREHOUSE = COMPUTE_WH TARGET_LAG = '1 hour'
                AS (SELECT transcript_text, customer_name, deal_stage, sales_rep, conversation_date
                    FROM SALES_INTELLIGENCE.DATA.SALES_CONVERSATIONS WHERE conversation_date >= '2024-01-01')"""
            if run_sql(sql):
                st.success("Search service created!")

with st.expander("Step 3: Create Semantic Model Stage"):
    st.markdown("Creates stage for the semantic model YAML file.")
    if st.button("Create Stage", key="step3"):
        if run_sql("CREATE STAGE IF NOT EXISTS SALES_INTELLIGENCE.DATA.MODELS"):
            st.success("Stage created!")
            st.info("Upload `sales_metrics_model.yaml` to @SALES_INTELLIGENCE.DATA.MODELS")

with st.expander("Step 4: Create Cortex Agent"):
    st.markdown("Creates the multi-tool agent with Search and Analyst capabilities.")
    if st.button("Create Agent", key="step4"):
        with st.spinner("Creating agent..."):
            sql = """CREATE OR REPLACE AGENT SALES_INTELLIGENCE.DATA.SALES_CONVERSATION_AGENT
            FROM SPECIFICATION $$
            models:
              orchestration: claude-sonnet-4-5
            instructions:
              response: 'You are a Sales Intelligence Assistant. Use SalesAnalyst for metrics, ConversationSearch for transcripts.'
              orchestration: 'For metrics (totals, counts), use SalesAnalyst. For conversations, use ConversationSearch.'
            tools:
              - tool_spec: {type: "cortex_search", name: "ConversationSearch", description: "Searches sales conversations"}
              - tool_spec: {type: "cortex_analyst_text_to_sql", name: "SalesAnalyst", description: "SQL queries on metrics"}
            tool_resources:
              ConversationSearch: {name: "SALES_INTELLIGENCE.DATA.SALES_CONVERSATION_SEARCH", max_results: "5"}
              SalesAnalyst:
                semantic_model_file: "@SALES_INTELLIGENCE.DATA.MODELS/sales_metrics_model.yaml"
...
with st.expander("Step 5: Verify Setup"):
    col1, col2 = st.columns(2)
    with col1:
        if st.button("Check Search Service"):
            results = run_sql("SHOW CORTEX SEARCH SERVICES IN SCHEMA SALES_INTELLIGENCE.DATA", True)
            st.dataframe(results) if results else None
    with col2:
        if st.button("Check Agent"):
            results = run_sql("SHOW AGENTS IN SCHEMA SALES_INTELLIGENCE.DATA", True)
            st.dataframe(results) if results else None
```
