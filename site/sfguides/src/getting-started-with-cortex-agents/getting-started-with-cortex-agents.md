...
language: en
summary: Build an intelligent sales assistant for Summit Gear Co. using Snowflake Cortex Agents with Cortex Code CLI, combining Cortex Search and Analyst for both structured and unstructured data.
environments: web
status: Published 
...
In this quickstart, you'll build an Intelligent Sales Assistant for **Summit Gear Co.** — a fictional outdoor adventure brand — that leverages Snowflake's capabilities for analyzing both sales conversations and deal metrics. Using **Cortex Code CLI** (Snowflake's AI-powered command-line assistant), we'll generate sample data, create search and analytics tools, build an agent, and test it — all from the terminal.
...
### What You'll Learn
- How to use Cortex Code CLI to generate and load sample data
...
After installation, launch Cortex Code and connect to your Snowflake account:
```bash
cortex
```

On first launch, Cortex Code will guide you through connecting to your Snowflake account.

## Generate Sample Data

We'll use Cortex Code to generate the complete Summit Gear Co. dataset. Paste the following prompt into Cortex Code:
...
Cortex Code will generate the database, tables, and all sample data. This may take a few minutes as it creates realistic conversation transcripts. When complete, verify the data:
...
Cortex Code will look up the correct syntax and execute the DDL. The search service builds a semantic index over all 150 transcripts, enabling natural language queries against the conversation content.
...
Cortex Code will load the `semantic-view` skill, which:
1. Runs FastGen to automatically generate a semantic model from the table metadata
...
Cortex Code will load the `cortex-agent` skill and:
...
Congratulations! You've built an Intelligent Sales Assistant for Summit Gear Co. entirely from Cortex Code CLI. The application demonstrates combining structured and unstructured data analysis through:
...
### What You Learned
- **Cortex Code CLI**: How to use Snowflake's AI-powered CLI to generate data, create services, and build agents conversationally
- **Cortex Search**: How to create a search service for hybrid semantic + keyword search over text data
- **Semantic Views**: How to use FastGen to automatically build semantic models from table metadata
