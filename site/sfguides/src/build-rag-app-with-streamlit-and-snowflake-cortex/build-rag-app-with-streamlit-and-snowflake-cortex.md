...
- Basic knowledge of Python and Streamli
...
```bash
git clone https://github.com/streamlit/30DaysOfAI.gi
cd 30DaysOfAI/app
```
...
with st.container(border=True):
    ...
    # Check if embeddings table exists and show record coun
    full_embedding_table = f"{st.session_state.day18_database}.{st.session_state.day18_schema}.{st.session_state.day18_embedding_table}"
                ...
                with st.expander(":material/search: View Individual Embedding Vectors"):
                    ...
                    if 'loaded_embedding' in st.session_state:
                        ...
                        if isinstance(emb_vector, str):
                            # If it's a string representation, parse i
                            import json
...
```python
from snowflake.core import Roo
...
2. **Augment**: Add retrieved chunks to the prompt as contex
3. **Generate**: LLM creates an answer grounded in the contex
...
```python
STRICT GUIDELINES:
1. ONLY use information from the provided customer review contex
...
```python
from trulens.core.otel.instrument import instrumen
...
- **Streamlit in Snowflake (SiS)**: [Create a Streamlit app](https://docs.snowflake.com/en/developer-guide/streamlit/getting-started/create-streamlit-ui) directly in Snowsigh
