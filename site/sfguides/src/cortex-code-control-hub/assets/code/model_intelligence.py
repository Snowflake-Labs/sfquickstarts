...
""")
        ...
        with col3:
            ...
            batch_size = int(get_app_setting(session, "EVAL_BATCH_SIZE", "1000"))
...
**Data source:** `CC_PROMPT_EVENTS` — populated nightly by `SP_CC_CLASSIFY_PROMPTS` which extracts `token_count.*` attributes from `SNOWFLAKE.LOCAL.AI_OBSERVABILITY_EVENTS` (`CodingAgent.Step-0` spans).
        ...
        if df_econ.empty:
            ...
            k1.metric("Total Tokens", _fmt_tokens(total_tokens),
                      help="input_tokens + output_tokens. Cache tokens are already included within input_tokens — they are NOT added on top. Formula: total = input + output.")
            ...
            k3.metric("Output Tokens", _fmt_tokens(int(df_econ["OUTPUT_T"].sum())),
                      help="Tokens generated by the LLM. Appears small (~0.5%) because each Step-0 span is a planning decision, not a full response.")
            k4.metric("Cache Hit Rate *(approx)*", f"{cache_hit_rate}%",
            ...
            k5.metric("Est. Credits Saved", f"{credits_saved:,.4f}",
                      help="Cache reads cost ~10x less than fresh input tokens. Savings = cache_read × (input_rate - cache_rate).")
            k6.metric("Cache Read Tokens", _fmt_tokens(total_cache_r),
                      help="Tokens served from the prompt cache (a subset of input_tokens). These cost 0.28 cr/1M vs 2.75 cr/1M for fresh input.")
...
""")
            ...
            with col_run:
                ...
                batch = int(get_app_setting(session, "EVAL_BATCH_SIZE", "1000"))
            ...
            batch_s = int(get_app_setting(session, "EVAL_BATCH_SIZE", "1000"))
