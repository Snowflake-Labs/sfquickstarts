...
def render(session):
    ...
    with st.container(border=True):
        st.subheader("Pricing Configuration",
                     help="Credit and USD pricing rates used for cost attribution estimates. "
                          "Standard Snowflake list price is $3.00/credit, but enterprise contracts may differ.")
        st.markdown("""
        The **USD per credit** rate is used to convert Cortex Code credit consumption into
        estimated dollar amounts on the Cost Attribution page. Update this to match your
        Snowflake contract rate. Standard list price is **$3.00/credit**.

        > Contact your Snowflake account team to confirm your contracted rate.
        """)
        current_usd = get_app_setting(session, "USD_PER_CREDIT", "3.00")
        usd_per_credit = st.number_input(
            "USD per Snowflake credit", min_value=0.01, max_value=20.0,
            value=float(current_usd), step=0.25, format="%.2f",
            key="setting_usd_credit",
            help="Snowflake standard list price is $3.00/credit. Enterprise accounts with committed spend "
                 "typically have lower contracted rates. This affects Est. USD figures on Cost Attribution."
        )
