author: mando222
id: deeptempo
summary: This is a guide on getting started with Deeptempo on Snowflake
...
# Getting Started with DEEPTEMPO (Using Sample Data)
<!-- ------------------------ -->
## Overview 
Duration: 1

This guide will walk you through the process of setting up and using the DEEPTEMPO Native App in your Snowflake environment with provided sample data.

### What You’ll Learn 
- How to run DeepTempo on sample data.
...
<!-- ------------------------ -->
## Install the DEEPTEMPO Native App
Duration: 2


1. Obtain the DEEPTEMPO native app from the Snowflake Marketplace.
...
```sql
GRANT CREATE COMPUTE POOL ON ACCOUNT TO APPLICATION DEEPTEMPO;
GRANT CREATE WAREHOUSE ON ACCOUNT TO APPLICATION DEEPTEMPO;
GRANT EXECUTE TASK ON ACCOUNT TO APPLICATION DEEPTEMPO;
GRANT EXECUTE MANAGED TASK ON ACCOUNT TO APPLICATION DEEPTEMPO;
```
...
GRANT USAGE ON WAREHOUSE <YOUR_WAREHOUSE_NAME> TO APPLICATION DEEPTEMPO;
```

The application comes with its own in-house warehouse (DEEPTEMPO_WH), which will be used for container services and live inference only.
...
```sql
CALL DEEPTEMPO.MANAGER.STARTUP();
```

Grant the DEEPTEMPO app the necessary permissions to access the sample data:

```sql
GRANT USAGE ON DATABASE deeptempo_sample TO APPLICATION DEEPTEMPO;
GRANT USAGE ON SCHEMA deeptempo_sample.inference_samples TO APPLICATION DEEPTEMPO;
GRANT USAGE ON SCHEMA deeptempo_sample.training_samples TO APPLICATION DEEPTEMPO;
GRANT SELECT ON ALL TABLES IN SCHEMA deeptempo_sample.inference_samples TO APPLICATION DEEPTEMPO;
GRANT SELECT ON ALL TABLES IN SCHEMA deeptempo_sample.training_samples TO APPLICATION DEEPTEMPO;
```

<!-- ------------------------ -->
## Perform Inference (DeepTempoInference)
Duration: 2

Use the `DEEPTEMPOINFERENCE` stored procedure to perform inference on sample static log data. It takes the table's FQN (fully qualified name), a Boolean to control whether full logs or only anomalies are returned, and the log type, in this order.

```sql
CALL DEEPTEMPO.DETECTION.DEEPTEMPOINFERENCE('<TABLE_FQN>',<RETURN BOOLEAN>, '<LOG_TYPE>');
```
...
```sql
CALL DEEPTEMPO.DETECTION.DEEPTEMPOINFERENCE('deeptempo_sample.inference_samples.workstations_inference', True, 'workstation');

```sql
CALL DEEPTEMPO.DETECTION.DEEPTEMPOINFERENCE('deeptempo_sample.inference_samples.webservers_inference',False,'webserver');
```
...
```sql
CALL DEEPTEMPO.FINE_TUNE.DEVICE_MODEL('<SERVICE_NAME>');
CALL DEEPTEMPO.FINE_TUNE.WORKSTATION_MODEL('<SERVICE_NAME>');
```
...
```sql
CALL DEEPTEMPO.FINE_TUNE.DEVICE_MODEL(‘first device model fine tuning’);
```
...
```sql
CALL DEEPTEMPO.LIVE_INFERENCE.START_LIVE_INFERENCE('<TABLE_PATH>', '<MODEL_TYPE>', ‘<REFRESH_TIME>’);
```
...
```sql
CALL DEEPTEMPO.LIVE_INFERENCE.START_LIVE_INFERENCE('mydb.myschema.mytable', 'workstation', ‘1 M’);
```
...
```sql
CALL DEEPTEMPO.LIVE_INFERENCE.SHUTDOWN_LIVE_INFERENCE('<TABLE_NAME>');
```
...
```sql
CALL DEEPTEMPO.LIVE_INFERENCE.SHUTDOWN_LIVE_INFERENCE('mytable');
```
...
```sql
SELECT * FROM DEEPTEMPO.LIVE_INFERENCE.LIVE_UPDATES_ON_<TABLE_NAME>;
```
...
```sql
SELECT * FROM DEEPTEMPO.LIVE_INFERENCE.LIVE_UPDATES_ON_mytable;
```
...
This guide uses sample data provided with DEEPTEMPO. For using your own data, please refer to the custom data guide.
...
Now it is time for you to try DeepTempo with your own data. The power is now at your fingertips to take on the future of security.


### What You Learned
- Completed setup of DeepTempo on SnowFlake
- Fine Tuned DeepTempo
- Ran DeepTempo on a sample dataset
