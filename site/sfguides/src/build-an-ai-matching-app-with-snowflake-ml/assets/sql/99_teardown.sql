...
-- 1. Drop SPCS inference services
DROP SERVICE IF EXISTS CC_DEMO.ML_REGISTRY.CC_MATCH_SERVICE;
DROP SERVICE IF EXISTS CC_DEMO.ML_REGISTRY.CC_MATCH_SERVICE_V2;
SELECT '[PASS] Inference services dropped' AS STATUS;

-- 2. Drop Cortex Search services
DROP CORTEX SEARCH SERVICE IF EXISTS CC_DEMO.RAW.CREATOR_CONTENT_SEARCH;
...
-- 4. Drop Model Monitor
DROP MODEL MONITOR IF EXISTS CC_DEMO.ML_REGISTRY.CREATOR_MATCH_MONITOR;
SELECT '[PASS] Model Monitor dropped' AS STATUS;

-- 5. Drop models from registry
...
-- 6. Drop enrichment tables and Dynamic Tables
