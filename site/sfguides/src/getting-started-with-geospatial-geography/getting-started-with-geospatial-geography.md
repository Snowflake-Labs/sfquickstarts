...
- **WKT & EWKT**: a "Well Known Text" string format for representing geospatial data and the "Extended" variation of that forma
- **WKB & EWKB**: a "Well Known Binary" format for representing geospatial data in binary and the "Extended" variation of that forma
...
// Define the query that represents the data outpu
...
// Order the results by the calculated distance and only return the lowes
order by 6 limit 1;
...
st_dwithin(coordinates,st_makepoint(-73.986226, 40.755702),1600) = true
order by 2 limit 1))
// Query the CTE result set, aggregating the coordinates into one objec
...
// Ordering by distance and only showing the lowes
order by 6 limit 1;
...
select st_makepolygon(st_makeline(st_collect(coordinates),
to_geography('POINT(-73.986226 40.755702)'))) as polygon from locations)
select sh.id,sh.coordinates,sh.name,sh.shop,sh.addr_housenumber,sh.addr_stree
