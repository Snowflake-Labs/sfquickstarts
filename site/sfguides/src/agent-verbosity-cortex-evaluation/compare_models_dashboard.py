...
MODELS = {
    "claude": {"name": "claude-3-5-sonnet", "provider": "cortex_sql", "label": "Claude 3.5 Sonnet (Cortex SQL)"},
    "mistral": {"name": "mistral-large2", "provider": "cortex_sql", "label": "Mistral Large 2 (Cortex SQL)"},
...
# Claude Code Agent Configuration (via Cortex Anthropic endpoint)
CLAUDE_CODE_CONFIG = {
    "endpoint": f"https://{get_account_from_toml()}.snowflakecomputing.com/api/v2/cortex/anthropic",
    "models": {
...
def get_account_from_toml(connection_name: str = "myaccount") -> str:
    import tomllib
    toml_path = os.path.expanduser("~/.snowflake/config.toml")
    try:
        with open(toml_path, "rb") as f:
            config = tomllib.load(f)
        if "connections" in config and connection_name in config["connections"]:
            return config["connections"][connection_name].get("account", "")
    except Exception:
        pass
    return ""

SNOWFLAKE_ACCOUNT = get_account_from_toml()
...
def call_model_rest_api(conn, model: str, prompt: str, system_prompt: str = None, max_tokens: int = 1024, temperature: float = 0.7):
    """Call model via Cortex REST API."""
    try:
        ...
        return {
            "response": f"REST API Exception: {str(e)}",
            "line_count": 0,
            "word_count": 0,
            "char_count": 0,
            "duration": 0,
...
def call_model(conn, model: str, verbosity: str, query: str, context: str = None, provider: str = "cortex_sql"):
    """Call model via Cortex SQL or REST API based on provider."""
    ...
    if provider == "cortex_rest":
        ...
        try:
            ...
            return {"response": str(e), "line_count": 0, "word_count": 0, "char_count": 0, "duration": 0, "success": False, "provider": "cortex_sql"}
        finally:
            cursor.close()

def run_claude_code_agent(prompt: str, model: str = None, pat_token: str = None, timeout: int = 120):
    """
    Run Claude Code agent in headless mode via Cortex Anthropic endpoint.
    ...
    # Build environment with Cortex Anthropic endpoint
    env = os.environ.copy()
    env["ANTHROPIC_BASE_URL"] = CLAUDE_CODE_CONFIG["endpoint"]
...
# Tabs - Added Persona Compare, Agent Verbosity, dbt Pipelines, TruLens Eval, A/B Testing, Inference Experiments, and Budget Monitor
...
with tab1:
    ...
    with col_cc_opt1:
        include_claude_code = st.checkbox(
            ...
            help="Run comparison with Claude Code agent via Cortex Anthropic endpoint"
        )
    ...
    if st.button("🚀 Run Comparison", type="primary"):
        ...
        if include_claude_code and not st.session_state.get("cc_pat_token"):
            ...
            # Calculate total including Claude Code if enabled
            cc_extra = 1 if include_claude_code else 0
            total = len(queries) * len(selected_verbosities) * (len(MODELS) + cc_extra)
            current = 0
        ...
        if not display_cols:
            ...
            for model_key in all_model_keys:
                ...
                if compliant_col in display_df.columns and lines_col in display_df.columns:
                    ...
                    if model_key == "claude_code":
                        column_config[status_col] = "Claude Code Agent (Cortex Anthropic)"
                    else:
...
with tab3:
    ...
    st.divider()
    st.markdown("### Claude Sonnet 4.5/4.6 (via Cortex Anthropic Endpoint)")
    st.caption(f"Endpoint: `{CLAUDE_CODE_CONFIG['endpoint']}`")
    
    col_cc1, col_cc2 = st.columns([2, 1])
    with col_cc1:
        # PAT Token - auto-load from config.toml
        if "cc_pat_token" not in st.session_state:
            st.session_state["cc_pat_token"] = get_pat_from_toml("myaccount")
        
        if st.session_state["cc_pat_token"]:
            st.success("✅ PAT loaded from ~/.snowflake/config.toml (myaccount)")
        else:
            pat_token = st.text_input(
                "Snowflake PAT Token:",
                value="",
                type="password",
                help="Personal Access Token from Snowflake for authentication",
                key="cc_pat_input"
            )
            st.session_state["cc_pat_token"] = pat_token
    ...
    if st.button("🤖 Run Claude via Cortex API", key="test_claude_code"):
        if not st.session_state.get("cc_pat_token"):
            st.error("Please provide a Snowflake PAT token")
        else:
            import requests
            ...
            with st.spinner(f"Calling {selected_model} via Cortex Anthropic API..."):
                url = f"{CLAUDE_CODE_CONFIG['endpoint']}/v1/messages"
                headers = {
                    "Content-Type": "application/json",
                    "Authorization": f"Bearer {st.session_state['cc_pat_token']}",
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.metric("Status", status)
                        st.caption(f"Response time: {duration:.2f}s | Model: {selected_model}")
                        
                        # Show usage if available
                        usage = data.get("usage", {})
                        if usage:
                            st.caption(f"Tokens - Input: {usage.get('input_tokens', 'N/A')}, Output: {usage.get('output_tokens', 'N/A')}")
...
with tab5:
    ...
    if st.button("🎭 Generate & Compare Personas", type="primary", key="persona_run"):
        ...
        if qa_pairs:
            ...
            with st.expander("📋 Generated Questions", expanded=True):
                ...
                st.dataframe(
                    qa_df[["domain", "category", "article", "question"]],
                    hide_index=True,
                    use_container_width=True
                )
    ...
    if "persona_results" in st.session_state and st.session_state["persona_results"]:
        ...
        if agg_dict and "persona_name" in persona_df.columns:
            ...
            st.dataframe(persona_df.head(10), hide_index=True, use_container_width=True)
            ...
            for agent_name in selected_agents:
                ...
                for model_key, model_config in MODELS.items():
                    ...
                    try:
                        ...
                        row[f"{model_key}_response"] = str(e)
...
with tab7:
    ...
    DBT_PROJECT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dbt_cortex_integration")
    ...
    with st.expander("🔍 Preview Pipeline Data", expanded=True):
        ...
        with col_wh:
            preview_warehouse = st.text_input(
                "Warehouse:",
                value="SNOW_INTELLIGENCE_DEMO_WH",
    ...
    **Snowflake AI Observability** enables evaluation of LLM responses using metrics.
    ...
    if not has_persona_results and not has_agent_results:
        ...
        if "eval_results" in st.session_state and st.session_state["eval_results"]:
            ...
            display_cols = ["source", "model", "question"] + [m for m in selected_metrics if m in eval_df.columns]
            st.dataframe(
...
COMPLETENESS: 0.X"""
            ...
            if "wiki_trulens_evals" in st.session_state:
                ...
                st.dataframe(wiki_eval_df.head(10), hide_index=True, use_container_width=True)
    ...
    if ab_test_type == "single":
        ...
        with col2:
            ...
            ab_judge = st.selectbox(
                ...
                options=["llama3.1-70b", "reka-flash", "mixtral-8x7b", "snowflake-arctic"],
                format_func=lambda x: {
                    "llama3.1-70b": "Llama 3.1 70B (Meta)",
                    "reka-flash": "Reka Flash (Reka AI)", 
                    "mixtral-8x7b": "Mixtral 8x7B (Mistral AI)",
    ...
    # ==================== MULTI-JUDGE PANEL ====================
        ...
        with col2:
            mj_judges = st.multiselect(
                ...
                options=["llama3.1-70b", "reka-flash", "mixtral-8x7b", "snowflake-arctic"],
                default=["llama3.1-70b", "reka-flash"],
                key="mj_judges",
...
with tab10:
    ...
    **LangGraph Agents + LangTrace + TruLens Evals + Snowflake ML Experiments**
                ...
                if tracking_enabled:
                    try:
                        ...
                        fresh_conn = snowflake.connector.connect(
                            account=config["connections"]["myaccount"].get("account", SNOWFLAKE_ACCOUNT),
    ...
    with col2:
        sae_sparsity = st.slider("Sparsity Target:", 0.01, 0.2, 0.05, key="sae_sparsity")
        sae_threshold = st.slider("Activation Threshold:", 0.0, 1.0, 0.5, key="sae_thresh")
    
    if st.button("🔍 Run SAE Analysis", key="run_sae"):
...
# ============== TAB 11: Budget Monitor ==============
with tab11:
    st.subheader("💰 Cortex REST API Budget Monitor")
    st.markdown("Monitor token consumption and set budget alerts for Cortex REST API usage")
    
    # Budget Configuration
    st.markdown("### ⚙️ Budget Configuration")
    
    col_budget1, col_budget2, col_budget3 = st.columns(3)
    
    with col_budget1:
        daily_budget = st.number_input("Daily Token Budget", min_value=0, value=1000000, step=100000, format="%d")
    with col_budget2:
        weekly_budget = st.number_input("Weekly Token Budget", min_value=0, value=5000000, step=500000, format="%d")
    with col_budget3:
    ...
    # Query KPI totals
    st.markdown("### 📊 Current Usage Overview")
    
    try:
        cursor = conn.cursor()
        
        # Get totals
        totals_df = cursor.execute("""
            SELECT 
        ...
        total_tokens = totals_df['TOTAL_TOKENS'].iloc[0] if not totals_df.empty else 0
        total_requests = totals_df['TOTAL_REQUESTS'].iloc[0] if not totals_df.empty else 0
        unique_models = totals_df['UNIQUE_MODELS'].iloc[0] if not totals_df.empty else 0
        unique_users = totals_df['UNIQUE_USERS'].iloc[0] if not totals_df.empty else 0
        
        with kpi1:
            st.metric("Total Tokens (30d)", f"{total_tokens:,.0f}" if total_tokens else "0")
        with kpi2:
            st.metric("Total Requests", f"{total_requests:,.0f}" if total_requests else "0")
        with kpi3:
            st.metric("Models Used", f"{unique_models:,.0f}" if unique_models else "0")
        with kpi4:
            st.metric("Unique Users", f"{unique_users:,.0f}" if unique_users else "0")
        ...
        # Get today's usage
        today_df = cursor.execute("""
            SELECT COALESCE(SUM(TOKENS), 0) AS today_tokens
            FROM SNOWFLAKE.ACCOUNT_USAGE.CORTEX_REST_API_USAGE_HISTORY 
            WHERE DATE(START_TIME) = CURRENT_DATE()
        """).fetch_pandas_all()
        
        today_tokens = today_df['TODAY_TOKENS'].iloc[0] if not today_df.empty else 0
        
        # Get this week's usage
        week_df = cursor.execute("""
            SELECT COALESCE(SUM(TOKENS), 0) AS week_tokens
            FROM SNOWFLAKE.ACCOUNT_USAGE.CORTEX_REST_API_USAGE_HISTORY 
            WHERE START_TIME >= DATE_TRUNC('week', CURRENT_TIMESTAMP())
        """).fetch_pandas_all()
        
        week_tokens = week_df['WEEK_TOKENS'].iloc[0] if not week_df.empty else 0
        
        col_status1, col_status2 = st.columns(2)
        
        with col_status1:
            daily_pct = (today_tokens / daily_budget * 100) if daily_budget > 0 else 0
            st.markdown(f"**Daily Budget:** {today_tokens:,.0f} / {daily_budget:,.0f} tokens")
            st.progress(min(daily_pct / 100, 1.0))
            if daily_pct >= alert_threshold:
                st.error(f"⚠️ Daily usage at {daily_pct:.1f}% - Alert threshold exceeded!")
            elif daily_pct >= 50:
                st.warning(f"Daily usage at {daily_pct:.1f}%")
            else:
                st.success(f"Daily usage at {daily_pct:.1f}%")
        
        with col_status2:
            weekly_pct = (week_tokens / weekly_budget * 100) if weekly_budget > 0 else 0
            st.markdown(f"**Weekly Budget:** {week_tokens:,.0f} / {weekly_budget:,.0f} tokens")
            st.progress(min(weekly_pct / 100, 1.0))
            if weekly_pct >= alert_threshold:
                st.error(f"⚠️ Weekly usage at {weekly_pct:.1f}% - Alert threshold exceeded!")
            elif weekly_pct >= 50:
                st.warning(f"Weekly usage at {weekly_pct:.1f}%")
            else:
                st.success(f"Weekly usage at {weekly_pct:.1f}%")
        ...
        if not model_df.empty:
            # Stacked bar chart using native Streamlit
            pivot_df = model_df.pivot(index='USAGE_DATE', columns='MODEL_NAME', values='TOTAL_TOKENS').fillna(0)
            st.bar_chart(pivot_df)
            
            # Model summary table
            st.markdown("### 📋 Usage by Model Summary")
            model_summary = cursor.execute("""
                SELECT 
                    MODEL_NAME,
                    SUM(TOKENS) AS total_tokens,
                    COUNT(*) AS request_count,
        ...
        st.warning("Unable to query Cortex REST API usage data.")
        st.code(str(e))
        st.info("""
        **Troubleshooting:**
        1. Ensure you have ACCOUNTADMIN role or appropriate privileges
        2. Grant access: `GRANT IMPORTED PRIVILEGES ON DATABASE SNOWFLAKE TO ROLE <your_role>;`
        3. The CORTEX_REST_API_USAGE_HISTORY view may have up to 2 hours of latency
        """)
    
    # Refresh button
    if st.button("🔄 Refresh Data", key="budget_refresh"):
        st.rerun()
