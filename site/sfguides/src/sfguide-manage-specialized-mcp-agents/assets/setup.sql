...
=============================================================================
  setup.sql
  Complete setup for MCP specialization demo.
  ...
  Run order:
    ...
    4. Semantic Views (8)
    5. MCP Servers (10 specialized + 1 enterprise monolith (24 tools) + 3 external connectors)
    6. Cortex Agents (4: 3 specialized + 1 enterprise monolith)
    7. RBAC Roles & Grants
=============================================================================
*/
...
SELECT 'Cortex Search services created' AS status;
...
-- ============================================================
-- STEP 4: MCP SERVERS
-- 10 specialized + 1 enterprise monolith (24 overlapping tools) + 3 external connectors
-- ============================================================
...
CREATE OR REPLACE MCP SERVER ACME_CORP.HR.enterprise_server
  FROM SPECIFICATION $$
    tools:
      - name: "handbook-search"
        type: "CORTEX_SEARCH_SERVICE_QUERY"
...
SELECT 'Internal MCP servers created: handbook_comp_server, benefits_server, org_server, budget_server, product_usage_server, invoice_search_server, spend_approvals_server, reporting_server, incident_server, infra_monitor_server, enterprise_server' AS status;

-- "External" connectors (Snowflake MCP servers disguised as SaaS integrations)

CREATE OR REPLACE MCP SERVER ACME_CORP.HR.workday_connect
  FROM SPECIFICATION $$
    tools:
      - name: "employee-directory"
        type: "CORTEX_ANALYST_MESSAGE"
        identifier: "ACME_CORP.HR.comp_semantic_view"
        title: "Workday Employee Directory"
        description: "Query employee records from Workday — headcount, departments, titles, levels, and hire dates."

      - name: "compensation-data"
        type: "CORTEX_ANALYST_MESSAGE"
        identifier: "ACME_CORP.HR.comp_semantic_view"
        title: "Workday Compensation Data"
        description: "Query compensation bands, salary ranges, and pay equity data from Workday HCM."

      - name: "benefits-handbook"
        type: "CORTEX_SEARCH_SERVICE_QUERY"
        identifier: "ACME_CORP.HR.handbook_search_svc"
        title: "Workday Benefits & Policies"
        description: "Search employee handbook for benefits and policies. Searchable text: content. Filterable attributes: title, section (Benefits, Career Development, Compensation, Compliance, Finance, Getting Started, IT & Security, Time Off, Work Arrangements). Returned columns: doc_id, title, section, content, last_updated. Use @eq filter on section to narrow results. Only @eq filters are supported — no @gte, @lte, @gt, @lt. For date questions, include date terms in the query text instead. Do NOT request columns not listed here."
...
CREATE OR REPLACE MCP SERVER ACME_CORP.FINANCE.salesforce_connect
  FROM SPECIFICATION $$
    tools:
      - name: "pipeline-analytics"
        type: "CORTEX_ANALYST_MESSAGE"
        identifier: "ACME_CORP.FINANCE.budget_semantic_view"
        title: "Salesforce Pipeline Analytics"
        description: "Query sales pipeline data — department revenue, budget allocation, and quarterly spend from Salesforce CRM."

      - name: "expense-tracker"
        type: "CORTEX_ANALYST_MESSAGE"
        identifier: "ACME_CORP.FINANCE.budget_semantic_view"
        title: "Salesforce Expense Tracker"
        description: "Query expense submissions, approval status, and spending by category from Salesforce."
...
CREATE OR REPLACE MCP SERVER ACME_CORP.IT.github_connect
  FROM SPECIFICATION $$
    tools:
      - name: "issue-tracker"
        type: "CORTEX_SEARCH_SERVICE_QUERY"
        identifier: "ACME_CORP.IT.incident_search_svc"
        title: "GitHub Issues & Incidents"
        description: "Search engineering issues and incident reports. Searchable text: description. Filterable attributes: severity (P1, P2, P3, P4), status (Open, Resolved, Investigating), title, assigned_to. Returned columns: incident_id, title, severity, status, description, assigned_to, created_date, resolved_date. Use @eq filter on severity or status to narrow results. Only @eq filters are supported — no @gte, @lte, @gt, @lt. For date questions, include date terms in the query text instead. Do NOT request columns not listed here."
  $$;

SELECT 'External connector MCP servers created: workday_connect, salesforce_connect, github_connect' AS status;

-- ============================================================
-- STEP 5: CORTEX AGENTS (MCP Clients)
-- 3 specialized + 1 enterprise monolith
...
CREATE OR REPLACE AGENT ACME_CORP.HR.enterprise_agent
  COMMENT = 'Enterprise monolith agent — 15 tools across all domains (same tools as specialized, single server)'
  PROFILE = '{"display_name": "Enterprise Assistant (Monolith)"}'
  FROM SPECIFICATION $$
  {
    "models": {"orchestration": "claude-sonnet-4-5"},
    "instructions": {
      "orchestration": "You are a general-purpose enterprise assistant for Acme Corp with access to HR, Finance, and IT tools. Choose the most appropriate tool for each question. Present data in markdown tables. Be concise.",
      "sample_questions": [
        {"question": "What is the total employee benefits cost by department?"},
        {"question": "Show me all pending invoices and pending spend approval requests. What is the total outstanding amount?"},
        {"question": "Which SLAs were breached in Q1 2025?"},
...
SELECT 'All agents created: hr_agent, finance_agent, it_agent, enterprise_agent' AS status;

-- ============================================================
-- STEP 7: RBAC ROLES & GRANTS
-- 4-layer model: Role -> Agent -> MCP Server -> Tool -> Data
-- ============================================================
...
GRANT USAGE ON MCP SERVER ACME_CORP.HR.workday_connect TO ROLE hr_analyst_role;
...
GRANT USAGE ON SEMANTIC VIEW ACME_CORP.FINANCE.budget_semantic_view TO ROLE finance_analyst_role;
GRANT USAGE ON SEMANTIC VIEW ACME_CORP.FINANCE.product_usage_semantic_view TO ROLE finance_analyst_role;
...
GRANT USAGE ON MCP SERVER ACME_CORP.FINANCE.salesforce_connect TO ROLE finance_analyst_role;
GRANT USAGE ON AGENT ACME_CORP.FINANCE.finance_agent TO ROLE finance_analyst_role;

-- Enterprise admin: inherits both + IT access
...
GRANT USAGE ON CORTEX SEARCH SERVICE ACME_CORP.IT.incident_search_svc TO ROLE enterprise_admin_role;
GRANT SELECT ON SEMANTIC VIEW ACME_CORP.IT.infra_semantic_view TO ROLE enterprise_admin_role;
GRANT SELECT ON SEMANTIC VIEW ACME_CORP.IT.sla_semantic_view TO ROLE enterprise_admin_role;
...
GRANT USAGE ON MCP SERVER ACME_CORP.IT.github_connect TO ROLE enterprise_admin_role;
GRANT USAGE ON MCP SERVER ACME_CORP.HR.enterprise_server TO ROLE enterprise_admin_role;
...
-- Grant enterprise_admin_role to ACCOUNTADMIN for demo
GRANT ROLE enterprise_admin_role TO ROLE ACCOUNTADMIN;

SELECT 'RBAC roles created: hr_analyst_role, finance_analyst_role, enterprise_admin_role' AS status;
...
--
-- -- RBAC Roles
-- DROP ROLE IF EXISTS enterprise_admin_role;
...
-- DROP AGENT IF EXISTS ACME_CORP.HR.enterprise_agent;
--
...
-- DROP MCP SERVER IF EXISTS ACME_CORP.HR.enterprise_server;
--
-- -- MCP Servers (Snowflake analytics)
...
-- DROP MCP SERVER IF EXISTS ACME_CORP.HR.workday_connect;
-- DROP MCP SERVER IF EXISTS ACME_CORP.FINANCE.salesforce_connect;
-- DROP MCP SERVER IF EXISTS ACME_CORP.IT.github_connect;
--
-- -- Semantic Views
