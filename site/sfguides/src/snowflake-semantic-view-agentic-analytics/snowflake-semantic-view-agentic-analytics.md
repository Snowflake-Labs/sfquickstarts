...
categories: snowflake-site:taxonomy/product/ai, snowflake-site:taxonomy/product/analytics, snowflake-site:taxonomy/solution-center/certification/quickstar
language: en
 ...
 * **AI Enhancement**: Auto-generate semantic views using Semantic View Autopilo
 * **Natural Language Querying**: Query data with plain English via Cortex Analys
...
use role agentic_analytics_vhol_role;
    ...
    -- Load Marketing Campaign Fac
    COPY INTO marketing_campaign_fac
    FROM @INTERNAL_DATA_STAGE/demo_data/marketing_campaign_fact.csv
    FILE_FORMAT = CSV_FORMAT
    ON_ERROR = 'CONTINUE';

    -- Load HR Employee Fac
    COPY INTO hr_employee_fac
  ...
  -- SELECT 'Internal Stage Files' as stage_type, COUNT(*) as file_count FROM (LS @INTERNAL_DATA_STAGE);
    ...
    -- Verify data loads
    SELECT 'DIMENSION TABLES' as category, '' as table_name, NULL as row_coun
    UNION ALL
    ...
    UNION ALL
    SELECT '', 'sales_fact', COUNT(*) FROM sales_fac
    UNION ALL
    SELECT '', 'finance_transactions', COUNT(*) FROM finance_transactions
    UNION ALL
    SELECT '', 'marketing_campaign_fact', COUNT(*) FROM marketing_campaign_fac
    UNION ALL
    SELECT '', 'hr_employee_fact', COUNT(*) FROM hr_employee_fac
    UNION ALL
    SELECT '', '', NULL
    UNION ALL
...
## AI-Powered Semantic View Enrichmen
...
### Generate Synthetic Query History for AI Enhancemen
...
SELECT
    d.DEPARTMENT_NAME,
    COUNT(DISTINCT f.EMPLOYEE_KEY) as employee_coun
...
SELECT
    ...
    SUM(f.ATTRITION_FLAG) as attrition_count,
    ROUND(SUM(f.ATTRITION_FLAG) * 100.0 / COUNT(*), 2) as attrition_rate_pc
FROM HR_EMPLOYEE_FACT f
JOIN DEPARTMENT_DIM d
    ON f.DEPARTMENT_KEY = d.DEPARTMENT_KEY
GROUP BY d.DEPARTMENT_NAME
...
SELECT
    ...
    COUNT(DISTINCT f.EMPLOYEE_KEY) as employee_count,
    AVG(f.SALARY) as avg_salary,
    SUM(f.ATTRITION_FLAG) as attrition_coun
FROM HR_EMPLOYEE_FACT f
JOIN LOCATION_DIM l
    ON f.LOCATION_KEY = l.LOCATION_KEY
...
SELECT
    ...
    ROUND(COUNT(DISTINCT f.EMPLOYEE_KEY) * 100.0 /
          SUM(COUNT(DISTINCT f.EMPLOYEE_KEY)) OVER (PARTITION BY d.DEPARTMENT_NAME), 2) as gender_pc
FROM HR_EMPLOYEE_FACT f
JOIN DEPARTMENT_DIM d
    ON f.DEPARTMENT_KEY = d.DEPARTMENT_KEY
JOIN EMPLOYEE_DIM e
...
SELECT
    ...
    ROUND((MAX(f.SALARY) - MIN(f.SALARY)) / MIN(f.SALARY) * 100, 2) as growth_pc
FROM HR_EMPLOYEE_FACT f
JOIN EMPLOYEE_DIM e
    ON f.EMPLOYEE_KEY = e.EMPLOYEE_KEY
JOIN DEPARTMENT_DIM d
...
```
# setup for AI-powered enrichmen
...
CORTEX_MODEL = 'claude-3-5-sonnet'  # Claude model with high token limi
...
if current_ddl and (unique_metrics or unique_dimensions):
    ...
    top_metrics = unique_metrics[:10]  # Top 10 most importan
    top_dimensions = unique_dimensions[:10]  # Top 10 most importan
...
- METRICS() format: table_name.metric_name AS AGG(expression) --- added with AI enhancemen
- FACTS() section is for table references, NOT aggregate expressions
- Non-aggregate column references go in DIMENSION sections
- DO NOT include any "WITH EXTENSION" section in the outpu
- Mark new additions with comment: --- added with AI enhancemen
...
METRICS (
  HR_EMPLOYEE_FACT.total_salary AS SUM(salary) --- added with AI enhancemen
)
...
def main():
    ...
    try:
        ...
        if selected_metric_full and selected_dimension_full:
            ...
            if sort_by == "Metric":
                sort_column = selected_metric_shor
            else:
                sort_column = selected_dimension_shor
    ...
    METRICS {selected_metric_full}
                ...
                if result and len(result) > 0:
                    ...
                    x_col = df.columns[0]
                    y_col = df.columns[1] if len(df.columns) > 1 else selected_metric_shor
...
def make_authenticated_request_via_session(session, url, method="POST", json_data=None, headers=None):
    ...
    """
    try:
        # Try to get the connection objec
        conn = None
        ...
        if hasattr(conn, '_request') or hasattr(conn, 'request'):
            ...
            if request_method:
                try:
                    # Try to make the reques
                    response = request_method(url, method=method, json=json_data, headers=headers)
                    return response
                except:
                    pass

        # Method 2: Check if there's an HTTP client or session objec
...
def get_auth_token(session):
    ...
    def _check_object_for_token(obj, depth=0, max_depth=3):
        ...
        # Check if it's a dict-like objec
    ...
    try:
        ...
        if conn:
            ...
            if hasattr(conn, '_rest'):
                rest_client = conn._res
                # Try direct attributes firs
...
def main():
    ...
    def get_semantic_views(schema_name):
        ...
        try:
            ...
            if '.' in schema_name:
                ...
                # Try to use current database contex
    ...
    if selected_view:
        ...
        # Natural language question inpu
            ...
            if not question.strip():
                ...
                try:
                    with st.spinner("🤖 Generating SQL from your question..."):
                        ...
                        # According to Snowflake Labs example: https://github.com/Snowflake-Labs/sfguide-getting-started-with-cortex-analys
                        ...
                        # Parse response
                        # Content is a string with serialized JSON objec
...
-- Create the agen
CREATE OR REPLACE AGENT SV_VHOL_DB.AGENTS.Agentic_Analytics_VHOL_Chatbo
...
- Mining query history for AI enhancemen
