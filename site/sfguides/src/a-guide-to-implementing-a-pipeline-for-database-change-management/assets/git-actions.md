...
1. **Environment Separation**: Non-prod uses `non_prod` configuration, production uses `prod` configuration
3. **Safety Checks**: PLAN always runs before DEPLOY to validate changes
4. **Branch Sync**: Automatic sync keeps develop up-to-date with main after releases
