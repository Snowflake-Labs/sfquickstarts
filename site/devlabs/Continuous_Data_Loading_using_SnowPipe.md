...
tags: Getting Started, AWS, SQL, Data Engineering
...
<!-- ------------------------ -->
## Select the Data Integration Trigger
Duration: 5

For Snowpipe to know a micro-batch is ready to load, a prompt needs to be pre-configured. This trigger could be a cloud storage notification, a REST API call to Snowpipe, or by utilizing Snowflake’s virtual warehouses.

1. Storage Infrastructure Notifications
2. Snowpipe’s REST API Endpoint
3. Snowflake’s Virtual Warehouse

![Snowflake_data-load-snowpipe-flow](assets/Snowflake_data-load-snowpipe-flow.png)

The image above reveals the Snowpipe workflows of three paths. In option one, cloud storage notifications from different IaaS stage data. Option two illustrates queueing data with a POST API call to the `insertFiles` endpoint. The POST call could come from an SDK or an AWS Lambda function. Lastly, option three pictures Snowflake’s virtual warehouses integrating files based on Snowpipe parameters.

This tutorial will take the first path, automating continuous data loading with cloud infrastructure notifications. In the next section, we’ll consider your cloud notification preferences.

<!-- ------------------------ -->
## Cloud Storage Notification Options
Duration: 5
...
1. Create Database, Table, Stage, and Pipe
...
You’ve learned the benefits of continuous data loading and the different ways Snowpipe can be triggered to load new data. Plus, you can now configure cloud storage event notifications and manage Snowpipe. Pat yourself on the back and consider the possibilities with data loading automation.
Review the various ways to implement Snowpipe. If you’re contemplating migrating an active database, select the staging method best for your architecture, and test the workflow on a small data scale. Experimenting will allow for an accurate cost-benefit analysis. If you aren’t gearing up to migrate, quickly fire up a [Java or Python SDK](https://docs.snowflake.com/en/user-guide/data-load-snowpipe-rest-gs.html) to see how Snowpipe uses REST API endpoints for data integration.